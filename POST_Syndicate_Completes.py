...
start_date = datetime(*time_data['start_date'])
...
def download_survey_data(survey_id, survey_type):
    ...
    if response.status_code == 200:
        # FIXED: Use 'latin-1' encoding with error handling, then normalize to UTF-8
        try:
        ...
        try:
            ...
            # Truncate the "Q0009017T" column to 90 characters
            ...
            # Replace 'nan' text with blank
            filtered_df = filtered_df.replace(r'\bnan\b', '', regex=True)

            # Use 'endtime' for the file date
            file_date = pd.to_datetime(filtered_df['endtime'].iloc[0])
            ...
            # FIXED: Save with UTF-8 encoding and error handling
...
try:
    ...
    if core_df is not None and core_csv_file is not None:
        ...
        if custom_df is not None and custom_csv_file is not None:
            ...
            core_date = pd.to_datetime(core_df['endtime'].iloc[0])
            custom_date = pd.to_datetime(custom_df['endtime'].iloc[0])
